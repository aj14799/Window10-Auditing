...
import pickle, os
import subprocess                         #To get Output of Windows Command Prompt
import re
import win32com.shell.shell as shell       #To run windows command in Shell with Administrative  Privillages
import time
...
class Password_Policy(Tk):      # Creating Windows Password Policies conif class

    def __init__(self):

        Tk.__init__(self)        # initiation of Tkinter window
        ...
        self.di = {
            ...
            'EnableGuestAccount': '0 (preferred disable)'}        # Values Should exist like ths
    ...
    def callp1(self, *arg):
        return self.txtp1.focus_set()         #Binding Entry
    ...
    def donep(self, *arg):             #Function Use to  Define Different Pass word Policies
        ...
        self.path = 'C:\\All1\\' + self.name1 + '.txt'               #Path Value
        ...
        try:
            self.command1 = 'mkdir C:\\All1'         #creating folder in c drive
            shell.ShellExecuteEx(lpVerb='runas', lpFile='cmd.exe', lpParameters='/c ' + self.command1) #Command to ge administrative privillages and run the command
        except:
            pass
        try:
            shell.ShellExecuteEx(lpVerb='runas', lpFile='cmd.exe', lpParameters='/c ' + self.commands)              #need to except error
        except:
        ...
        with open(self.path, 'a+', encoding='utf-16-le') as file:      #openinf file
            file.seek(self.startpos, 0)
            while self.counter <= self.lno:
                line = file.readline().strip()          #Saperating words
                if line != '':
                    self.ls.append(line.split(' = '))
                    self.counter += 1
        self.ls = dict(self.ls) #converting  into dictionarary
...
#############################################################################################################
        ...
        self.ab = []
        self.ab.append('|{:_^28}|{:_^16}|{:_^74}|'.format('Keys', 'UserValues', 'RecommendedValues'))  # Appending value in list in a particular format
        for a in self.di.keys():
            self.ab.append('|{:^30}|{:^16}|{:<72}|,'.format(a, self.ls[a], self.di[a]))
        T = tk.Text(self, height=200, width=126,bg='#000000', fg='#006400')      #Creating Text Box and printing value
...
class MsConfig(Tk):        #Microsoft Configruation class
    def __init__(self, *arg):
        ...
        # Creating Functions for msconfig Commands

    def back(self):
        self.destroy()
        win1()

    def simplify_G(self, a, list_no=0, value_no=0, no=0, rm_ls_no=0, rm_ls_keys=[], rm_ls2_no=0, rm_ls2_keys=[]): #It is self made advanced func to create table 
        ls1 = a.split(' \n\n')
        ls1.remove('\n')
        # except:
        #   pass
...
############################################### 
    ...
    def group_info(self):
        ...
        try:
            ...
            T = tk.Text(self, height=200, width=200, bg='#000000', fg='#FFFFFF')
            T.pack()
            T.insert(tk.END, a)
    ...
    def net_info(self):
        ...
        print(tkmessagebox.showinfo('Following Options are available: ',di))
        ab = []
        T = tk.Text(self, height=200, width=200,  fg='#FFFFFF')
        T.insert(tk.END,'Following Options are available: ', di)
        for c in di.keys():
            #ab.append('{:<3}: {:20}'.format(c, di[c]))
            T.insert(tk.END, '{:<3}: {:20}'.format(c, di[c]))
        #T = tk.Text(self, height=200, width=126, bg='#000000', fg='#006400')
        T.pack()
        #T.see(tkinter.END)
        #T.insert(tk.END, ab)
